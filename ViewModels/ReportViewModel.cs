...
namespace IEC60335Develop.ViewModels {
    public class ReportViewModel : BindableBase {
        ...
        private void SortOutputClick() {
            ...
            string dataHeader = "日期,时间,电流,功率";
            sw.WriteLine(dataHeader);
            for (int j = 0; j < WTReportModel.PowerList.Count; j++) {
                sw.WriteLine($"{DateTime.Now.ToString("yyyy/MM/dd")},{ElementsCollection[j].Timestamp},{WTReportModel.CurrentList[j]},{WTReportModel.PowerList[j]}");
            }
            sw.Flush();
            sw.Close();
        }
